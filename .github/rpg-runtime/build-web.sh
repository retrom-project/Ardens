...
work=$(mktemp -d "$root/.retrom-build/retrom-ardens-web.XXXXXX")
...
if ! docker run --rm --platform linux/amd64 --hostname retrom-ardens \
  ...
  /recipe/build-emulatorjs-core.sh ardens \
...
stage="$work/stage"
mkdir -p "$stage"
install -m 0644 "$work/raw/ardens_libretro.js" "$stage/"
install -m 0644 "$work/raw/ardens_libretro.wasm" "$stage/"
cat "$root/LICENSE.txt" "$work/raw/retroarch-COPYING" > "$stage/license.txt"
printf '%s\n' '{"minimumEJSVersion":"4.2.2","version":"1.18"}' > "$stage/build.json"
printf '%s\n' '{"name":"ardens","extensions":["hex","arduboy"],"makeoptions":{"buildpath":"./","makescript":"CMakeLists.txt","arguments":[]},"options":{},"save":true,"license":"LICENSE","repo":"https://github.com/retrom-project/Ardens"}' > "$stage/core.json"
chmod 0644 "$stage/build.json" "$stage/core.json" "$stage/license.txt"

(cd "$stage" && 7z a -mtm=off -mta=off -mtc=off -bd -bso0 -bsp0 -t7z "$output/ardens-wasm.data" \
  ardens_libretro.js ardens_libretro.wasm build.json core.json license.txt)
